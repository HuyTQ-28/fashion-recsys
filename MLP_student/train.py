...
def train_student(model, loader, epochs=10, lr=1e-3, patience=3, device='cuda', use_wandb=False):
    """
    Huấn luyện mô hình Student MLP với embeddings CLIP -> HGNN.
    
    model: mô hình StudentMLP
    loader: DataLoader trả về (clip, hgnn)
    epochs: số epoch tối đa
    lr: learning rate
    patience: early stopping patience
    device: 'cuda' hoặc 'cpu'
    use_wandb: ghi log lên wandb nếu True
    """
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch}: trung bình alignment loss = {avg_loss:.6f}")
