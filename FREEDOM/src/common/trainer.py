   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model):
        ...
        self.saved_model_file = os.path.join(
            self.checkpoint_dir,
            '{}-{}-{}.pth'.format(config['model'], config['dataset'], get_local_time())
        )
    ...
    def _save_checkpoint(self, epoch_idx, valid_result, test_result):
        ...
        checkpoint = {
            ...
            'best_test_upon_valid': test_result,
            'config': getattr(self.config, 'final_config_dict', {}),
        }
        torch.save(checkpoint, self.saved_model_file)
        return self.saved_model_file
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if update_flag:
                    ...
                    if saved:
                        saved_model_file = self._save_checkpoint(epoch_idx, valid_result, test_result)
                        update_output += '\nSaving current best model to {}'.format(saved_model_file)
                    if verbose:
