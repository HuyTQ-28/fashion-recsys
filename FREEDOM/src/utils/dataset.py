...
class RecDataset(object):
    ...
    def _check_expected_metadata(self):
        expected_user_num = self.config['expected_num_users']
        expected_item_num = self.config['expected_num_items']
        expected_inter_num = self.config['expected_num_interactions']
        expected_split_counts = self.config['expected_split_counts']

        if expected_user_num and self.user_num != expected_user_num:
            raise ValueError(
                'Dataset has {} users, expected {}.'.format(self.user_num, expected_user_num)
            )
        if expected_item_num and self.item_num != expected_item_num:
            raise ValueError(
                'Dataset has {} items, expected {}.'.format(self.item_num, expected_item_num)
            )
        if expected_inter_num and len(self.df) != expected_inter_num:
            raise ValueError(
                'Dataset has {} interactions, expected {}.'.format(len(self.df), expected_inter_num)
            )
        if expected_split_counts:
            split_counts = self.df[self.splitting_label].value_counts().to_dict()
            actual = [int(split_counts.get(i, split_counts.get(float(i), 0))) for i in range(3)]
            if actual != expected_split_counts:
                raise ValueError(
                    'Dataset split counts are {}, expected {}.'.format(actual, expected_split_counts)
                )
