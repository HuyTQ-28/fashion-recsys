...
def build_sim(context):
    context_norm = context.div(torch.norm(context, p=2, dim=-1, keepdim=True).clamp(min=1e-12))
    sim = torch.mm(context_norm, context_norm.transpose(1, 0))
    return sim

class LATTICE(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim, weight_size, dropout_list, image_feats, text_feats, cache_dir=None):
        super().__init__()
        ...
        cache_dir = cache_dir or os.path.join('..', 'data', args.dataset, '%d-core' % args.core)
        if not os.path.exists(cache_dir):
            os.makedirs(cache_dir)
        image_adj_file = os.path.join(cache_dir, 'image_adj_%d.pt' % args.topk)
        text_adj_file = os.path.join(cache_dir, 'text_adj_%d.pt' % args.topk)
