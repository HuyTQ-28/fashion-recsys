...
from utility.load_data import Data, resolve_dataset_path
...
args = parse_args()
Ks = eval(args.Ks)
cores = args.eval_cores if args.eval_cores > 0 else multiprocessing.cpu_count()
cores = max(1, min(cores, multiprocessing.cpu_count()))

dataset_path = resolve_dataset_path(args.data_path, args.dataset)
data_generator = Data(path=dataset_path, batch_size=args.batch_size)
...
def get_performance(user_pos_test, r, auc, Ks):
    precision, recall, ndcg, hit_ratio, map_score = [], [], [], [], []
    pos_len = len(user_pos_test)

    for K in Ks:
        precision.append(metrics.precision_at_k(r, K))
        recall.append(metrics.recall_at_k(r, K, pos_len))
        ndcg.append(metrics.ndcg_at_k(r, K, pos_len))
        hit_ratio.append(metrics.hit_at_k(r, K))
        map_score.append(metrics.map_at_k(r, K, pos_len))

    return {'recall': np.array(recall), 'precision': np.array(precision),
            'ndcg': np.array(ndcg), 'hit_ratio': np.array(hit_ratio),
            'map': np.array(map_score), 'auc': auc}
...
def test_torch(ua_embeddings, ia_embeddings, users_to_test, is_val, drop_flag=False, batch_test_flag=False):
    ...
    u_batch_size = EVAL_BATCH_SIZE
    i_batch_size = EVAL_BATCH_SIZE
